<template>
    <div class="container">
        <AssignmentHeader :assignment="asssignment" />
        <!-- <header>
            <h1 class="text-center font-extrabold text-4xl mt-20 mb-5">{{ asssignment.title }}</h1>

            <div class="flex justify-center gap-x-8">
                <div><span class="font-semibold">Deadline:</span> <span class="text-primary font-semibold">{{ asssignment.deadline?.readable }}</span></div>
                <div><span class="font-semibold">Počet bodov:</span> <span class="text-primary font-semibold">{{ asssignment.points?.readable }}</span></div>
                <div><span class="font-semibold">Zverejnené:</span> <span class="text-sliver">{{ asssignment.published_at?.readable }}</span></div>
            </div>
        </header> -->
        <nav class="my-20">
        ...
        <main class="pb-20">
            <Transition name="fade" mode="out-in">
                ...
                <MaterialsSection v-else-if="currentSection == 'materials'" :materials="asssignment?.materials" />
            </Transition>
        </main>
    </div>
</template>
...
const getAssignment = async () => {
    ...
    asssignment.value = res.data.data;
}
